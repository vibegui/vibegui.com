...
```bash
node --experimental-strip-types --experimental-sqlite scripts/build.ts --mode=pages
```

This works because:
1. **Node 22** is available on Cloudflare (with experimental TypeScript + SQLite)
2. **Vite assets are pre-committed** in `dist/` (built locally before push)
3. **SQLite database is committed** in `data/content.db`
...
Check that:
1. `data/content.db` is committed and up to date
2. `generate.ts` ran successfully (check build logs for article count)
3. The article slug matches (trailing slashes are stripped)

### Context pages 404
Same as articles - check that:
1. `context/*.md` files exist
2. Build logs show context count > 0

### Assets look broken
The `dist/` directory with Vite-built assets needs to exist. Either:
1. Run `bun run build` locally before pushing, OR
2. Ensure `dist/assets/` is committed (not recommended)
