...
async function main() {
  ...
  // Read the manifest generated by Vite plugin (has hashes, committed to git)
  const sourceManifestPath = resolve(ROOT, "public/content/manifest.json");
  if (!existsSync(sourceManifestPath)) {
    console.error("❌ No manifest found at public/content/manifest.json");
  ...
  for (const article of manifest.articles) {
    ...
    if (!existsSync(sourceFile)) {
      console.warn(`  ⚠️  Source not found: ${article.id}.md`);
      continue;
    }
    ...
    writeFileSync(destPath, content);
    console.log(`  📄 ${article.id}.md → ${hashedName}`);
  ...
  console.log(`  ✅ Injected manifest path: /content/${manifestFileName}`);
